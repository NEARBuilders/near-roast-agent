...
async function triggerAnalysis(accountId: string, accessToken: string): Promise<string> {
  ...
  if (line.startsWith('data: ')) {
    ...
    try {
      ...
      switch (currentEventType) {
        ...
        case 'error':
          result.status = 'error';
          throw new StreamProcessingError(parsedData.message || 'Unknown error occurred');
        default:
...
async function processEventStream(
  ...
  try {
    while (result.status !== 'completed' && result.status !== 'error') {
      ...
      [buffer, currentEventType] = await processBuffer(buffer, result, currentEventType);
    }
  } catch (error) {
    throw new StreamProcessingError(error instanceof Error ? error.message : 'Stream processing failed');
  }
}
...
function validateFinalResult(result: AnalysisResult): void {
  ...
  throw new StreamProcessingError(`Stream ended before receiving: ${missing.join(', ')}`);
}
...
async function streamSummary(taskId: string, accessToken: string): Promise<AnalysisResult> {
  ...
  try {
    ...
    if (result.status === 'completed') {
      if (!result.robustSummary || !result.shortSummary) {
        throw new StreamProcessingError('Task completed but missing required data');
      }
      return result;
    }
